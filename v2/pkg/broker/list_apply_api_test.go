...
func (s *ListApplySuite) TestApplyCases(c *gc.C) {
	...
	var fetchRev = func(name pb.Journal) int64 {
		var resp, err = rjc.List(ctx, &pb.ListRequest{
	...
	c.Check(must(rjc.Apply(ctx, &pb.ApplyRequest{
		Changes: []pb.ApplyRequest_Change{
			{Upsert: &specB, ExpectModRevision: fetchRev("journal/B")},
		},
	})).Status, gc.Equals, pb.Status_OK)
	...
	c.Check(must(rjc.Apply(ctx, &pb.ApplyRequest{
		Changes: []pb.ApplyRequest_Change{
			{Delete: "journal/A", ExpectModRevision: fetchRev("journal/A")},
		},
	})).Status, gc.Equals, pb.Status_OK)
	...
	c.Check(must(rjc.Apply(ctx, &pb.ApplyRequest{
		Changes: []pb.ApplyRequest_Change{
			{Delete: "journal/B", ExpectModRevision: fetchRev("journal/B") - 1},
		},
	...
	c.Check(must(rjc.Apply(ctx, &pb.ApplyRequest{
		Changes: []pb.ApplyRequest_Change{
			{Upsert: &specB, ExpectModRevision: fetchRev("journal/B") - 1},
		},
	})).Status, gc.Equals, pb.Status_ETCD_TRANSACTION_FAILED)
